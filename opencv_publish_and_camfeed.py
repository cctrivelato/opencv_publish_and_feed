...
def initialize_camera(camera_id):
    ...
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # Lower resolution to reduce load
...
# Function to publish and delete count data to MySQL database that is linked to the Grafana Dashboards (Non-blocking using threading)
def grafana_database(people_count, station, time_spent, status, previous_status, sfvis, presence_rate, presence_total):
    def publish():
        db_config = {
            'user': 'administrator',
            'password': '******',
        ...
        try:
            ...
            if time_spent:
                query = ("INSERT INTO sfvis0" + str(sfvis) +
            ...
            # Query to delete the oldest item from the database
            cursor.execute("DELETE FROM sfvis0" + str(sfvis) + "_grafanadata ORDER BY Timestamp ASC LIMIT 1")
...
def check_status(people_count, station, status, time_started, previous_status, sfvis, presence_total, presence_rate):
    ...
    if status != previous_status:
        ...
        if status == "Occupied" and previous_status == "Vacant":
            ...
            grafana_database(people_count, station, time_spent, status, previous_status, sfvis, presence_rate, presence_total)
...
def main():
    ...
    camera_id2 = 4  # This value only changes for correct port selection of the camera
    ...
    while True:
        ...
        # Check for status and publish information to the database
	    ...
	    # Add a small sleep to prevent CPU overloading
        time.sleep(0.01)
        ...
        if not pause:
            if (check_time % 20) == 0:
                ...
                grafana_database(people_count1, station1, None, status1, previous_status1, sfvis, presence_rate1, presence_total1)
                publish_to_mysql(people_count2, station2, None, status2, previous_status2, sfvis, presence_rate2, presence_total2)
                grafana_database(people_count2, station2, None, status2, previous_status2, sfvis, presence_rate2, presence_total2)
                pause = True
